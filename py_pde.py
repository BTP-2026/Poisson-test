...
class PdeModel:
    ...
    def __init__(self, inputs, outputs, get_models, loss_fn, optimizer, metrics, parameters,umin, umax,bound_w,
        ...
        self.inner_data = self.create_data_pipeline(inputs['xin'], inputs['fin'], inputs['xbc'], inputs['ubc'],batch=batches).cache()
        self.bound_data = self.create_data_pipeline(inputs['xb'], inputs['fb'], outputs['ub'], inputs['xbc_b'], inputs['ubc_b'],batch=batches).cache()
        # self.val_data = self.create_data_pipeline(inputs['xval'], inputs['fv'], inputs['xbc_val'], inputs['ubc_val'], batch=val_batches).cache()
    ...
    def Pde_residual(self, input_data, training=True):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            with tf.GradientTape(persistent=True) as tape2:
                tape2.watch(x)
                u = self.nn_model([x, f, xbc, ubc], training=training)
    ...
    def test_step(self, inp_data):
        ...
        upred = self.nn_model([x, f, xbc, ubc], training=False)
    ...
    def run(self, epochs, log_dir, wb=False, validation_freq=1000):
        ...
        plt.plot(range(1, len(odata) + 1), np.log(odata['residual_loss']), label='Residual Loss')
        plt.plot(range(1, len(odata) + 1), np.log(odata['bound_loss']), label='Boundary Loss')
        plt.xlabel('Epochs')
        plt.ylabel('Log_Losses')
        plt.title('log losses plot')
        plt.legend()
        plt.savefig(log_dir + '_log_losses_plt.png', dpi=300)
